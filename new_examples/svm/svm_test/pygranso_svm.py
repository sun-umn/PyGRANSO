...
name_str = "{}_{}{}_total{}_maxtime{}".format(data_name,square_str,maxfolding,total,maxclocktime) + data_num
...
zeta_plot = np.reshape(np.array([zeta_list]),(-1))

matplot.subplots(figsize=(10, 5))
matplot.plot(zeta_plot, sorted_test_acc,'-gD' ,color='red' , label="Testing Accuracy")
matplot.plot(zeta_plot, sorted_acc,'-gD' , label="Training Accuracy")
#matplot.xticks(L,L)
matplot.grid(True)
matplot.xlabel("zeta")
matplot.ylabel("Accuracy")
matplot.legend()
matplot.title('Accuracy versus the slack variable Zeta')

png_title =  "png/" + date_time + name_str
matplot.savefig(os.path.join(my_path, png_title))

print("successful rate = {}".format(len(zeta_plot)/total))
