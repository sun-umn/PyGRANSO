...
def spectral_radius():
    ...
    file = f"{currentdir}/examples/data/spec_radius_opt_data.mat"
...
def dictionary_learning():
    ...
    Y = norm.ppf(np.random.rand(n, m)) * (
        norm.ppf(np.random.rand(n, m)) <= theta
...
def robust_PCA():
    ...
    opts = pygransoStruct()
    opts.print_frequency = 10
    opts.x0 = 0.2 * torch.ones((2 * d1 * d2, 1)).to(device=device, dtype=torch_dtype)
    opts.opt_tol = 1e-6
    opts.maxit = 50
    opts.print_level = print_level
    opts.double_precision = double_precision
...
def lasso():
    ...
    )  # double precision requireed in torch operations
...
if __name__ == "__main__":
    ...
    try:
        ...
        print()
        print(
            f"\033[93m Test {count} fail, please carefully read the instructions on https://ncvx.org/ for installation \033[0m"
        )
        print()
        traceback.print_exc()
