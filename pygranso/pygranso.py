...
def pygranso(var_spec, combined_fn, user_opts=None):
    ...
    try:
        ...
        if nn_model is not None:
            ...
            if opts.is_backtrack_linesearch:
    ...
    def msg_box_fn(
        margin_spaces, title_top, title_bottom, msg_lines, sides=True, user_width=0
    ):
        return pMB.printMessageBox(
            opts.print_ascii,
    ...
    def print_notice_fn(title, msg):
        return msg_box_fn(2, title, "", msg, True)
    ...
    def printSummary(name, fieldname):
        return printSummaryAux(name, fieldname, soln, printer)
...
def getBfgsManager(opts, torch_device, double_precision):
    if opts.limited_mem_size == 0:
        ...
        def get_bfgs_fn(H, scaleH0):
            return bfgsHI.bfgsHessianInverse(H, scaleH0)
    else:

        def get_bfgs_fn(H, scaleH0):
            return lbfgsHI.bfgsHessianInverseLimitedMem(
                H,
                scaleH0,
...
def failedToBracketedMinimizerMsg(soln):
    if hasattr(soln, "mu_lowest"):
        s_mu = [
            f"PyGRANSO attempted mu values down to {soln.mu_lowest} unsuccessively.  However, if ",
            "the objective function is indeed bounded below on the feasible set, ",
            f"consider restarting PyGRANSO with opts.mu0 set even lower than {soln.mu_lowest}.",
        ]
    else:
        s_mu = [""]
