...
from pygranso.exceptions.exceptions import PyGransoSystemExitException
from pygranso.private import bfgsHessianInverse as bfgsHI
from pygranso.private import bfgsHessianInverseLimitedMem as lbfgsHI
from pygranso.private import printMessageBox as pMB
...
def pygranso(
    ...
    try:
        if nn_model is not None:
            ...
            # call the functions getNvar to get the total
    ...
    except PyGransoSystemExitException:
...
def progressSlowMsg(constrained):
    if constrained:
        s = (
            "relative decrease in penalty function is below tolerance and"
            "feasibility tolerances satisfied."
        )
    else:
