...
from pygranso.private import formatOrange as fO
from pygranso.private import truncate as TR
from pygranso.private.centerString import centerString
from pygranso.private.printMessageBox import truncateStr
...
class tP:
    ...
    def tablePrinter(
        self, use_ascii, use_orange, labels, widths, spacing, span_labels=None
    ):
        """
        tablePrinter:
        ...
        if np.all(self.use_orange is None):
            self.use_orange = False

        [self.header, self.rules, self.row_str, self.msg_str, self.msg_width] = init(
            use_ascii, labels, widths, spacing, span_labels
        )

        oP = Class_overlayPrinter()
        [self.printOverlayFn, overlay_width] = oP.overlayPrinter(
            self.rules.m.rs, self.msg_width
        )
        ...
        setattr(
            printer, "overlayOrange", lambda s: self.printOverlay(self.use_orange, s)
        )
    ...
    def printHeader(self):
        ...
        if (
            self.last_printed == "r"
            or self.last_printed == "h"
            or self.last_printed == "w"
        ):
            r = self.rules.m.rh
    ...
    def printMessage(self, use_orange, msg_lines):
        ...
        if use_orange:
            ...
            def msg_line_fn(s):
                return self.messageLineOrange(s)
        else:

            def msg_line_fn(s):
                return self.messageLineBlack(s)
...
class Class_overlayPrinter:
    def overlayPrinter(self, rule, msg_width):
        ...
        def print_overlay_fn():
            return self.printOverlay()
    ...
    def printOverlayLine(self, str):
        ...
        print(
            self.rule[0:3] + "   " + str + "   " + self.rule[self.max_width + 10 :],
            end="",
        )
...
def processLabel(label, width, add_arrows):
    ...
    if add_arrows and len(label.strip()) > 0:
        ...
        def process_fn(x):
            return addArrows(x, width)
    else:

        def process_fn(x):
            return centerString(x, width)
...
def processSpannedLabels(labels, widths, span_labels, row_str, vs):
    ...
    for j in range(n_multicols):
        ...
        widths_list[col_start] = getTotalWidth(
            widths, delim_width, col_start + 1, col_end
        )
        widths = widths_list
...
def init(use_ascii, labels, widths, spacing, span_labels):
    ...
    if np.any(span_labels is not None):
        [labels, row_str] = processSpannedLabels(
            labels, widths, span_labels, row_str, vs
        )
...
def addArrows(label, width):
    ...
    if freespaces < 0:
        ...
        label = (
            "<"
            + "-" * int(arrow_length_left)
            + " "
            + label
            + " "
            + "-" * int(arrow_length_right)
            + ">"
        )
    else:
...
def makeRules(use_ascii, h2):
    ...
    if use_ascii:
        [hs, hd, cs, cd, csd, cds, tsdl, tdl, tds, td, tdsu, tdu, edtr, edbr] = (
            getSymbolsASCII()
        )
    else:
        [hs, hd, cs, cd, csd, cds, tsdl, tdl, tds, td, tdsu, tdu, edtr, edbr] = (
