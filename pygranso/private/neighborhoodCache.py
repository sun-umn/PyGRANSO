...
class nC:
    ...
    def neighborhoodCache(self, max_size, radius):
        ...
        neighborhoodCache:
            ...
            neighborhoodCache.py (introduced in PyGRANSO v1.0.0)
            Copyright (C) 2016-2021 Tim Mitchell

            This file is a direct port of neighborhoodCache.m, which is included as part
        ...
        self.distances = torch.ones(
            (1, self.max_size), device=self.torch_device, dtype=self.torch_dtype
        ) * float("inf")
        self.samples = None
        ...
        def get_neighborhood_fn(x, x_data):
            return self.getCachedNeighborhoodAbout(x, x_data)
    ...
    def getCachedNeighborhoodAbout(self, x, x_data):
        if self.last_added_ind == 0:
            ...
            indx = torch.logical_and(
                self.distances > self.radius, self.distances != float("inf")
            )
            # indx                = self.distances > self.radius and  not np.isinf(self.distances)
            computed = torch.sum(torch.sum(indx)).item()
            self.distances[indx] = torch.sqrt(
                torch.sum(torch.square(self.samples[:, indx[0]]))
            )
