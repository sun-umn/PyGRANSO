...
def pygransoPrinterColumns(opts, ineq_constraints, eq_constraints):
    ...
    pygransoPrinterColumns:
        ...
        This is a direct port of pygransoPrinterColumns.m from GRANSO v1.6.4.
...
def getNumberColumn(label, num_width, nonnegative=False):
    ...
    if nonnegative:
        ...
        def format_number_fn(x):
            return dropFirstChar(get_num_fn(x))
...
class gSD:
    ...
    def getSearchDirection(self, constrained, opts):
        ...
        if opts.use_orange and not opts.use_ascii:
            ...
            def format_fn(sd_code, random_attempts):
                return self.formatSearchDirectionOrange(
                    sd_code, random_attempts, constrained
                )
        else:

            def format_fn(sd_code, random_attempts):
                return self.formatSearchDirection(sd_code, random_attempts)
...
class gSM:
    ...
    def getStationarityMeasure(self, use_orange, use_ascii):
        ...
        if use_orange and not use_ascii:
            ...
            def format_fn(value, stat_type):
                return self.formatStationarityMeasureOrange(value, stat_type)
        else:

            def format_fn(value, stat_type):
                return self.formatStationarityMeasure(value, stat_type)
