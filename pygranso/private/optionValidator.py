...
from pygranso.private.isColumn import isColumn
from pygranso.private.isFiniteValued import isFiniteValued
from pygranso.private.isMbyN import isMbyN
from pygranso.private.isPositiveDefinite import isPositiveDefinite
from pygranso.private.isRealValued import isRealValued
from pygranso.private.isRestartData import isRestartData
from pygranso.private.isRow import isRow
from pygranso.private.isString import isString
from pygranso.private.isTorchDevice import isTorchDevice
from pygranso.private.makeStructWithFields import makeStructWithFields
from pygranso.pygransoStruct import pygransoStruct


def _is_zero(x):
    """Return True if x is identically zero (scalar or all elements zero)."""
    if hasattr(x, "__iter__") and not isinstance(x, (str, bytes)):
        try:
            return (
                bool(torch.all(x == 0).item())
                if torch.is_tensor(x)
                else bool(np.all(np.asarray(x) == 0))
            )
        except Exception:
            return x == 0
    return x == 0
...
class oV:
    ...
    def optionValidator(
        self, program_name, default_opts, sub_validators=None, sub_optname=None
    ):
        """
        optionValidator:
            ...
            This file is a direct port of optionValidator.m, which is included as part
        ...
        #  Mandatory Options
        assert isinstance(self.program_name, str), (
            self.id_str + "Input ''program_name'' must be a string!"
        )
        assert isinstance(self.default_opts, pygransoStruct), (
            self.id_str
            + "Input argument ''default_opts'' must be an pygransoStruct class!"
        )
        ...
        if self.sub_validators is not None:
            assert isinstance(self.sub_validators, pygransoStruct), (
                self.id_str
                + "Input argument ''sub_validators'' must be an pygransoStruct class!"
            )
        ...
        if sub_optname is not None:
            assert isinstance(sub_optname, str), (
                self.id_str + "Input ''sub_optname'' must be a string!"
            )
        ...
        setattr(
            validator,
            "setUserOpts",
            lambda some_user_opts: self.setUserOpts(some_user_opts),
        )
        setattr(
            validator, "assert", lambda tf, error_msg: self.customAssert(tf, error_msg)
        )
        setattr(validator, "isSpecified", lambda name: self.isSpecified(name))
        setattr(
            validator,
            "validateValue",
            lambda opt_name, validate_fn, error_msg: self.validateValue(
                opt_name, validate_fn, error_msg
            ),
        )
        setattr(
            validator,
            "validateAndSet",
            lambda opt_name, validate_fn, error_msg: self.validateAndSet(
                opt_name, validate_fn, error_msg
            ),
        )
        ...
        setattr(
            validator,
            "setStructWithFields",
            lambda name, varargin: self.setStructWithFields(name, varargin),
        )
        setattr(
            validator,
            "setStructWithValidation",
            lambda name: self.setStructWithValidation(name),
        )
        setattr(
            validator, "setFunctionHandle", lambda name: self.setFunctionHandle(name)
        )
        setattr(validator, "setInteger", lambda name: self.setInteger(name))
        setattr(
            validator, "setIntegerPositive", lambda name: self.setIntegerPositive(name)
        )
        setattr(
            validator, "setIntegerNegative", lambda name: self.setIntegerNegative(name)
        )
        setattr(
            validator,
            "setIntegerNonnegative",
            lambda name: self.setIntegerNonnegative(name),
        )
        setattr(
            validator,
            "setIntegerNonpositive",
            lambda name: self.setIntegerNonpositive(name),
        )
        setattr(
            validator,
            "setIntegerInRange",
            lambda name, l, r: self.setIntegerInRange(name, l, r),
        )
        ...
        setattr(
            validator, "setRealNonnegative", lambda name: self.setRealNonnegative(name)
        )
        setattr(
            validator, "setRealNonpositive", lambda name: self.setRealNonpositive(name)
        )
        setattr(
            validator,
            "setRealInIntervalOO",
            lambda name, l, r: self.setRealInIntervalOO(name, l, r),
        )
        setattr(
            validator,
            "setRealInIntervalOC",
            lambda name, l, r: self.setRealInIntervalOC(name, l, r),
        )
        setattr(
            validator,
            "setRealInIntervalCO",
            lambda name, l, r: self.setRealInIntervalCO(name, l, r),
        )
        setattr(
            validator,
            "setRealInIntervalCC",
            lambda name, l, r: self.setRealInIntervalCC(name, l, r),
        )
        ...
        setattr(
            validator,
            "setRealFiniteValued",
            lambda name: self.setRealFiniteValued(name),
        )
        ...
        setattr(
            validator,
            "setRowDimensioned",
            lambda name, dim: self.setRowDimensioned(name, dim),
        )
        setattr(validator, "setColumn", lambda name: self.setColumn(name))
        setattr(
            validator,
            "setColumnDimensioned",
            lambda name, dim: self.setColumnDimensioned(name, dim),
        )
        setattr(
            validator,
            "setDimensioned",
            lambda name, m, n: self.setDimensioned(name, m, n),
        )
        setattr(validator, "setSparse", lambda name: self.setSparse(name))
        setattr(
            validator,
            "setPositiveDefinite",
            lambda name: self.setPositiveDefinite(name),
        )
    ...
    def setUserOpts(self, some_user_opts):
        ...
        assert isinstance(some_user_opts, pygransoStruct), (
            self.id_str + f"{self.ov_str}.setUserOpts(s) requires that s is a struct."
        )
    ...
    def validateValue(self, opt_name, validate_fn, error_msg):
        ...
        assert hasattr(self.default_opts, opt_name), (
            self.id_str + self.unknown_str + opt_name
        )
        if hasattr(self.user_opts, opt_name):
            value = getattr(self.user_opts, opt_name)
            #  make sure the user's value is not empty
            assert np.any(value is not None), self.invalid_str + opt_name + " nonempty"
    ...
    def isSpecified(self, name):
        tf = hasattr(self.user_opts, name) and getattr(self.user_opts, name) is not None
        return tf
    ...
    def setFunctionHandle(self, name):
        self.validateAndSet(
            name, lambda x: isinstance(x, types.LambdaType), "a function handle"
        )
        return
    ...
    def setStructWithFields(self, name, varargin):
        self.validateAndSet(
            name,
            lambda x: isinstance(x, pygransoStruct)
            or isinstance(x, pygransoStruct)
            and [hasattr(x, field) for field in varargin],
            "a struct with fields: {}".format(", ".join(varargin)),
        )
    ...
    def setStructWithValidation(self, name):
        if self.isSpecified(name):
            self.setStruct(name)
            validated_sub_opts = getattr(self.sub_validators, name)(
                getattr(self.user_opts, name)
            )
    ...
    def setIntegerPositive(self, name):
        self.validateAndSet(
            name, lambda x: isAnInteger(x) and x > 0, "a positive integer"
        )
        return

    def setIntegerNegative(self, name):
        self.validateAndSet(
            name, lambda x: isAnInteger(x) and x < 0, "a negative integer"
        )
        return

    def setIntegerNonnegative(self, name):
        self.validateAndSet(
            name, lambda x: isAnInteger(x) and x >= 0, "a nonnegative integer"
        )
        return

    def setIntegerNonpositive(self, name):
        self.validateAndSet(
            name, lambda x: isAnInteger(x) and x <= 0, "a nonpositive integer"
        )
        return
    ...
    def setIntegerInRange(self, name, l, r):
        self.validateAndSet(
            name,
            lambda x: isAnInteger(x) and l <= x and x <= r,
            f"an integer in {{{l:g},...,{r:g}}}",
        )
        return
    ...
    def setRealPositive(self, name):
        self.validateAndSet(
            name, lambda x: isARealNumber(x) and x > 0, "a positive real number"
        )
        return

    def setRealNegative(self, name):
        self.validateAndSet(
            name, lambda x: isARealNumber(x) and x < 0, "a negative real number"
        )
        return

    def setRealNonnegative(self, name):
        self.validateAndSet(
            name, lambda x: isARealNumber(x) and x >= 0, "a nonnegative real number"
        )
        return

    def setRealNonpositive(self, name):
        self.validateAndSet(
            name, lambda x: isARealNumber(x) and x <= 0, "a nonpositive real number"
        )
        return

    def setRealInIntervalOO(self, name, l, r):
        self.validateAndSet(
            name, lambda x: isARealNumber(x) and l < x and x < r, f"in ({l:g},{r:g})"
        )
        return
    ...
    def setRealInIntervalCC(self, name, l, r):
        self.validateAndSet(
            name,
            lambda x: isARealNumber(x) and l <= x and x <= r,
            f"in [{l:g},{r:g}]",
        )
        return
    ...
    def setRealFiniteValued(self, name):
        self.validateAndSet(
            name,
            lambda x: isRealValued(x) and isFiniteValued(x),
    ...
    def setFiniteValued(self, name):
        self.validateAndSet(
            name, lambda x: isFiniteValued(x), "finite valued (no nans/infs allowed)"
        )
        return
    ...
    def setRowDimensioned(self, name, dim):
        self.validateAndSet(
            name,
            lambda x: isRow(x) and x.size == dim,
            "a row vector of length %d" % dim,
        )
        return
    ...
    def setColumnDimensioned(self, name, dim):
        self.validateAndSet(
            name,
            lambda x: isColumn(x) and torch.numel(x) == dim,
            "a column vector of length %d" % dim,
        )
        return
    ...
    def setRestartData(self, name):
        self.validateAndSet(
            name,
            lambda x: isRestartData(x),
            "lbfgs_warm_start required form {'S':matrix, 'Y':matrix, 'rho':row vector, 'gamma':scalar}",
        )
        return
    ...
    def setPositiveDefinite(self, name):
        self.validateAndSet(
            name,
            lambda x: isFiniteValued(x) and isPositiveDefinite(x),
    ...
    def checkAndSetSubValidator(self, name):
        assert hasattr(self.default_opts, name), (
            self.id_str + f"Sub-validator {name} is missing from default_opts!"
        )
        assert isinstance(getattr(self.sub_validators, name), types.FunctionType), (
            self.id_str + "Sub-validator %s must be a function handle!" % +name
        )
        try:
