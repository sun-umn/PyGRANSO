...
class oV:
    ...
    def setUserOpts(self, some_user_opts):
        ...
        assert isinstance(some_user_opts, pygransoStruct), (
            self.id_str + "%s.setUserOpts(s) requires that s is a struct." % self.ov_str
        )
    ...
    def setStructWithFields(self, name, varargin):
        self.validateAndSet(
            ...
            "a struct with fields: %s" % ", ".join(varargin),
        )
    ...
    def setRealInIntervalOO(self, name, l, r):
        self.validateAndSet(
            name, lambda x: isARealNumber(x) and l < x and x < r, "in (%g,%g)" % (l, r)
        )
        return
    ...
    def checkAndSetSubValidator(self, name):
        assert hasattr(self.default_opts, name), (
            self.id_str + "Sub-validator %s is missing from default_opts!" % name
        )
