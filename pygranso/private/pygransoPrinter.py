...
from pygranso.private import (
    pygransoConstants as pC,
)
from pygranso.private import (
    pygransoPrinterColumns as pPC,
)
...
class pgP:
    ...
    def pygransoPrinter(self, opts, n, n_ineq, n_eq):
        ...
        setattr(
            print_opts, "ls_max_estimate", 50 * (self.opts.linesearch_reattempts + 1)
        )
        ...
        if constrained:
            ...
            self.pen_vals_fn = lambda penfn_at_x: self.penaltyFunctionValuesConstrained(
                penfn_at_x
            )
            if n_ineq and n_eq:
                self.viol_vals_fn = lambda penfn_at_x: self.violationValuesBoth(
                    penfn_at_x
                )
            elif n_ineq:
                self.viol_vals_fn = lambda penfn_at_x: self.violationValuesInequality(
                    penfn_at_x
                )
            else:
                self.viol_vals_fn = lambda penfn_at_x: self.violationValuesEquality(
                    penfn_at_x
                )
        ...
        labels = (
            ...
            self.eq_c.label,
            self.sd_c.label,
            self.ls_evals_c.label,
            self.ls_size_c.label,
            self.ngrad_c.label,
            self.stat_c.label,
        )
        ...
        widths = (
            ...
            self.eq_c.width,
            self.sd_c.width,
            self.ls_evals_c.width,
            self.ls_size_c.width,
            self.ngrad_c.width,
            self.stat_c.width,
        )
        ...
        tP_obj = tP()
        self.table_printer = tP_obj.tablePrinter(
            ascii, use_orange, labels, widths, 1, span_labels
        )

        def msg_box_fn(varargin):
            return pMB.printMessageBox(ascii, use_orange, varargin)
        ...
        setattr(
            printer,
            "init",
            lambda penfn_at_x, stat_value, n_qps: self.init(
                penfn_at_x, stat_value, n_qps
            ),
        )
        setattr(
            printer,
        ...
        setattr(
            printer, "summary", lambda name, penfn_at_x: self.summary(name, penfn_at_x)
        )
        setattr(printer, "unscaledMsg", lambda: self.unscaledMsg())
        setattr(
            printer,
            "lineSearchRestart",
            lambda iter, mu_ls: self.lineSearchRestart(iter, mu_ls),
        )
        setattr(printer, "bfgsInfo", lambda iter, code: self.printBfgsInfo(iter, code))
        setattr(
            printer, "regularizeError", lambda iter: self.printRegularizeError(iter)
        )
        setattr(
            printer, "qpError", lambda iter, err, loc: self.printQPError(iter, err, loc)
        )
        setattr(
            printer, "quadprogFailureRate", lambda rate: self.quadprogFailureRate(rate)
        )
    ...
    def init(self, penfn_at_x, stat_value, n_qps):
        ...
        self.table_printer.row(
            (self.iter_c.format_fn(0),)
            + pen_values
            + (self.obj_c.format_fn(penfn_at_x.f),)
            + viol_values
            + info_values
    ...
    def iteration(
        ...
        pen_values = self.pen_vals_fn(penfn_at_x)
        viol_values = self.viol_vals_fn(penfn_at_x)
        info_values = self.infoValues(
            sd_code, random_attempts, ls_evals, alpha, n_grads
        )

        self.table_printer.row(
            (self.iter_c.format_fn(iter),)
            + pen_values
            + (self.obj_c.format_fn(penfn_at_x.f),)
            + viol_values
            + info_values
    ...
    def summary(self, name, penfn_at_x):
        ...
        info_values = (
            self.sd_c.blank_str,
            self.ls_evals_c.blank_str,
            self.ls_size_c.blank_str,
            self.ngrad_c.blank_str,
        )

        self.table_printer.row(
            (self.iter_c.format_str(name),)
            + (self.mu_c.blank_str,)
            + (self.pen_c.blank_str,)
            + (self.obj_c.format_fn(penfn_at_x.f),)
            + viol_values
            + info_values
            + (self.stat_c.blank_str,)
        )
    ...
    def penaltyFunctionValuesConstrained(self, penfn_at_x):
        pen_args = (
            self.mu_c.format_fn(penfn_at_x.mu),
            self.pen_c.format_fn(penfn_at_x.p.item()),
        )
        return pen_args
    ...
    def violationValuesBoth(self, penfn_at_x):
        viol_args = (
            self.ineq_c.format_fn(penfn_at_x.tvi),
            self.eq_c.format_fn(penfn_at_x.tve),
        )
        return viol_args

    def infoValues(self, sd_code, random_attempts, ls_evals, alpha, n_grads):
        info_args = (
            self.sd_c.format_fn(sd_code, random_attempts),
            self.ls_evals_c.format_fn(ls_evals),
            self.ls_size_c.format_fn(alpha),
            self.ngrad_c.format_fn(n_grads),
        )
        return info_args

    def lineSearchRestart(self, iter, mu_ls):
        msg = [
            "Line search failed to bracket a minimizer, ",
    ...
    def printQPError(self, iter, err, loc):
        ...
        f"PyGRANSO: QUADPROG ERROR IN {loc} QP - END"
    ...
    def gransoHeader(self):
        ...
        def spec_fn(s, n):
            return " %-35s: %*d" % (f"# of {s}", w, n)

        msg = [
            "Problem specifications:",
            spec_fn("variables", self.n),
            spec_fn("inequality constraints", self.n_ineq),
            spec_fn("equality constraints", self.n_eq),
        ]
        self.table_printer.msg(msg)
        ...
        msg = [
            "Limited-memory mode enabled with size = %d." % nvec,
...
def quadprogFailureRateMsg(rate):
    s = [
        "WARNING: PyGRANSO''s performance may have been hindered by issues with QP solver.",
        f"quadprog''s failure rate: {rate}%",
