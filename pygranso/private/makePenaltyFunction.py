...
class PanaltyFuctions:
    ...
    def makePenaltyFunction(
        ...
        makePenaltyFunction:
            ...
            makePenaltyFunction.py (introduced in PyGRANSO v1.0.0)
            Copyright (C) 2016-2021 Tim Mitchell

            This is a direct port of makePenaltyFunction.m from GRANSO v1.6.4.
        ...
        [
            ...
            self.x,
            ineq_fn,
            lambda x, fn: self.evalInequality(x, fn),
            True,
        ...
        [
            ...
            self.x,
            eq_fn,
            lambda x, fn: self.evalEquality(x, fn),
            False,
            prescaling_threshold,
            torch_device,
            double_precision,
        ...
        if isinstance(self.tv_l1_grad, int):
            ...
            self.p_grad = self.mu * self.f_grad + self.tv_l1_grad.reshape(
                self.f_grad.shape
            )
        ...
        setattr(
            ...
            lambda x_in, get_grad: self.evaluateAtX(x_in, get_grad),
        )
        setattr(
            penalty_fn_object, "updatePenaltyParameter", update_penalty_parameter_fn
        )
        setattr(penalty_fn_object, "getX", lambda: self.getX())
        setattr(
            penalty_fn_object, "getComponentValues", lambda: self.getComponentValues()
        )
        setattr(
            penalty_fn_object,
            "getPenaltyFunctionValue",
            lambda: self.getPenaltyFunctionValue(),
        )
        setattr(
            penalty_fn_object, "getPenaltyParameter", lambda: self.getPenaltyParameter()
        )
        setattr(penalty_fn_object, "getGradients", lambda: self.getGradients())
        setattr(
            penalty_fn_object,
            "getNumberOfEvaluations",
            lambda: self.getNumberOfEvaluations(),
        )
        setattr(penalty_fn_object, "getBestSolutions", get_best_fn)
        setattr(
            penalty_fn_object, "isFeasibleToTol", lambda: self.isFeasibleToTolerances()
        )
        ...
        setattr(
            penalty_fn_object,
            "restoreSnapShot",
            lambda user_snap_shot=None: self.restoreSnapShot(user_snap_shot),
        )
        setattr(
            penalty_fn_object,
            "addStationarityMeasure",
            lambda stationarity_measure: self.addStationarityMeasure(
                stationarity_measure
            ),
        )
    ...
    def evalInequality(self, x, fn):
        ...
        [self.tvi, self.tvi_l1, self.tvi_l1_grad] = totalViolationInequality(
            self.ci, self.ci_grad
        )
        return

    def evalEquality(self, x, fn):
        [self.ce, self.ce_grad] = fn(x)
        [self.tve, self.tve_l1, self.tve_l1_grad] = totalViolationEquality(
            self.ce, self.ce_grad
        )
        return
    ...
    def updateBestSoFarConstrained(self):
        ...
        update_mf = self.tv < self.most_feasible.tv or (
            self.tv == self.most_feasible.tv and self.f < self.most_feasible.f
        )
    ...
    def unscaleFieldsConstrained(self, data):
        ...
        unscaled = dataStructConstrained(
            ...
            tve_u,
            tv_u,
            self.is_feasible_to_tol_fn(tvi_u, tve_u),
            data.mu,
        )
        return unscaled
...
def assertFnOutputs(n, f, g, fn_name):
    ...
    assertFn(
        torch.isreal(f) if torch.is_tensor(f) else np.isreal(f),
        arg1,
        fn_name,
...
class Class_splitEvalAtX:
    ...
    def splitEvalAtX(self, eval_at_x_fn, x0):
        ...
        [f, f_grad, self.ci, self.ci_grad, self.ce, self.ce_grad] = self.eval_at_x_fn(
            x0
        )
        ...
        eq_fn = (
            (lambda varargin: self.equality(varargin))
            if (torch.is_tensor(self.ce))
            else (None)
        )
...
def totalViolationEquality(ce, ce_grad):
    ...
    # l_1 penalty term for penalty function
    tve_l1 = torch.sum(ve)
    tve_l1_grad = torch.sum(ce_grad[:, indx[:, 0]], 1) - torch.sum(
        ce_grad[:, (~indx[:, 0])], 1
    )
