...
def printMessageBox(
    ...
    printMessageBox:
        ...
        printMessageBox.py (introduced in PyGRANSO v1.0.0)
        Copyright (C) 2016-2021 Tim Mitchell

        This file is a direct port of printMessageBox.m, which is included as part
    ...
    if not have_user_width:
        ...
        def trunc_fn(s):
            return truncateStr(s, max_line_width)
    ...
    def add_title_fn(h, t):
        return addTitle(h, t, TITLE_INDENT, TITLE_MARGIN)
    ...
    if use_orange and not use_ascii:
        ...
        def print_fn(strPrint):
            return pO.printOrange(strPrint)
    else:

        def print_fn(strPrint):
            return print(strPrint, end="")

    def print_line_fn(str_tmp):
        return print_fn(format_str % (str_tmp))
...
def addTitle(header, title, indent, spaces):
    ...
    if len(title) != 0:
        ...
        header_new = (
            header[0 : indent + 1] + title_str + header[indent + 1 + title_chars :]
        )
        return header_new
    else:
        return header
