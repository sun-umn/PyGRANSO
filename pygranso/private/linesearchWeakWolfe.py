...
def linesearchWeakWolfe(
    ...
    linesearchWeakWolfe:
        ...
        linesearchWeakWolfe.py (introduced in PyGRANSO v1.0.0)
        Copyright (C) 2016-2021 James Burke, Adrian Lewis, Buyun Liang,
        Tim Mitchell, and Michael Overton

        This file is a MATLAB-to-Python port of linesearchWeakWolfe.m from
        GRANSO v1.6.4 with the following new functionality and/or changes:
            1. Backtracking line search and related option added
            2. Modified the interface to remove several unused outputs.
    ...
    xalpha = x0  # OPTIMIZED: No clone needed, just reference
    falpha = f0
    gradalpha = grad0.detach()  # OPTIMIZED: Only detach, no clone needed
    ...
    while (beta - alpha) > (
        torch.norm(x0 + alpha * d).item() / dnorm
    ) * step_tol and n_evals < maxit:
        x = x0 + t * d
        ...
        if is_feasible and not np.isnan(f) and f <= fvalquit and not np.isinf(f):
            ...
            xalpha = x  # OPTIMIZED: No clone needed
            [f, grad, is_feasible] = obj_fn(x)
            falpha = f
            gradalpha = grad.detach()  # OPTIMIZED: Only detach
            return [alpha, xalpha, falpha, gradalpha, fail]

        #  the first condition must be checked first. NOTE THE >=.
        if f >= f0 + c1 * t * g0 or np.isnan(
            f
        ):  # first condition violated, gone too far
            beta = t

        elif (
            not is_backtrack_linesearch and gtd <= c2 * g0 or torch.isnan(gtd)
        ):  # second condition violated, not gone far enough
            alpha = t
            xalpha = x  # OPTIMIZED: No clone needed
            falpha = f
            gradalpha = grad.detach()  # OPTIMIZED: Only detach
